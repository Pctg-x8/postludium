...
fn main()
{
	...
	devconf::load_configurations(std::borrow::Cow::Borrowed(&arg), |name|
		std::fs::File::open(base_path.join(name)).and_then(|mut fp| { let mut s = String::new(); fp.read_to_string(&mut s).map(|_| s) }), &mut ColoredReporter);
}
